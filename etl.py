...
def inserir_dim_tempo(df, database_url, tabela_destino="dim_tempo", schema="DW"):
    ...
    with engine.begin() as conn:  # Transação gerenciada pelo SQLAlchemy
        try:
            ...
            print(f"{len(df_to_insert)} novos registros inseridos na tabela {schema}.{tabela_destino}.")
        except Exception as e:
...
def inserir_fato(df, database_url, tabela_destino, schema='DW', cod_sp=None, cod_sb=None, cod_gs=None, 
                 _cod_sp=None, _cod_sb=None, _cod_gs=None, 
    ...
    if not registros_filtrados.empty:
        ...
        required_columns = [cod_sp, cod_sb, cod_gs, 'id_tempo', 
        ...
        if missing_columns:
            ...
            return  # Ou trate o erro como preferir
        ...
        # Especificar a query SQL para inserção
        query_insert = text(f"""
            INSERT INTO "{schema}".{tabela_destino} ("{_cod_sp}", "{_cod_sb}", "{_cod_gs}", "id_tempo", 
            "{_vl_empenhado}", "{_vl_liquidado}", "{_vl_pago}", 
            "{_vl_rp_inscrito}", "{_vl_rp_cancelado}", "{_vl_rp_pago}")
            VALUES (:cod_sp, :cod_sb, :cod_gs, :id_tempo, :vl_empenhado, :vl_liquidado, :vl_pago, 
            :vl_rp_inscrito, :vl_rp_cancelado, :vl_rp_pago)
        """)
...
def tratar_registros(df):
    ...
    codigos = ['Código Órgão Superior', 'Código Órgão Subordinado', 'Código Unidade Gestora']
...
def filtrar_dados_novas_datas(df, database_url, schema=None, tabela_fatos=None, tabela_dim=None):
    ...
    # Remover duplicatas com base na coluna `_cod`
    #df = df.drop_duplicates(subset=['Ano e mês do lançamento'])
...
def etl(inicio_ano=2022, fim_ano=datetime.now().year):
    ...
    df_novos_dados = filtrar_dados_novas_datas(df, database_url, 'DW', 'fato_gastos', 'dim_tempo')
    if not df_novos_dados.empty:
        inserir_fato(
            df_novos_dados, 
            database_url, 
            tabela_destino='fato_gastos', 
            schema='DW', 
